...
const menuGroups = [
  {
    ...
    items: [
      ...
      { to: "/admin/appointments", label: "Lịch hẹn", icon: "LH", badgeKey: "appointments" },
...
const ADMIN_SEEN_UNAVAILABLE_KEY = "admin_seen_unavailable_time_id";

function AdminLayoutWithAlerts() {
  ...
  useEffect(() => {
    ...
    const fetchSidebarAlerts = async () => {
      const [appointmentResponse, unavailableResponse] = await Promise.allSettled([
        ...
        axiosClient.get("/dentist-unavailable-times/recent"),
      ]);

      if (!isMounted) return;

      const pendingAppointments =
        appointmentResponse.status === "fulfilled"
          ? (appointmentResponse.value.data.data || []).filter(
              (appointment) => appointment.status === "Pending",
            )
          : [];
      const recentUnavailableItems =
        unavailableResponse.status === "fulfilled"
          ? unavailableResponse.value.data.data || []
          : [];

      if (location.pathname.startsWith("/admin/dentists")) {
        localStorage.setItem(
          ADMIN_SEEN_UNAVAILABLE_KEY,
          String(getMaxId(recentUnavailableItems)),
        );
      }

      setSidebarAlerts({
        appointments: pendingAppointments.length,
        dentists: location.pathname.startsWith("/admin/dentists")
          ? 0
          : countItemsAfterSeenId(recentUnavailableItems, ADMIN_SEEN_UNAVAILABLE_KEY),
      });
    };

    fetchSidebarAlerts();
    window.addEventListener("admin-sidebar-alerts-refresh", fetchSidebarAlerts);
    const intervalId = window.setInterval(fetchSidebarAlerts, 30000);

    return () => {
      isMounted = false;
      window.removeEventListener("admin-sidebar-alerts-refresh", fetchSidebarAlerts);
  ...
  const renderBadge = (item) => {
    ...
    return (
      <span className="sidebar-alert-badge" title={item.badgeTitle || "Có việc mới cần xử lý"}>
        {sidebarAlerts[item.badgeKey] > 9 ? "9+" : sidebarAlerts[item.badgeKey]}
      </span>
    );
  };
    ...
    <div className="admin-layout admin-shell-v2">
      <aside className="admin-sidebar admin-sidebar-v2">
        ...
        <div className="admin-sidebar-footer">
          <button type="button" className="admin-website-button" onClick={() => navigate("/")}>
            Xem website
          </button>
...
export default AdminLayoutWithAlerts;
